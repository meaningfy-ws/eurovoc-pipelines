...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="moduleReference" select="'curr'"/>
    <!--    rdfs:label -->
...
<xsl:variable name="ontologyLabelShapes" select="'Core Public Organisation Vocabulary - core shapes'"/>
    ...
    <xsl:variable name="incompatibleWith" select="'3.1.2'"/>
    <!--    owl:versionInfo -->
    <xsl:variable name="versionInfo" select="'4.1.1'"/>
    <!--    bibo:status-->
    <xsl:variable name="ontologyStatus" select="'Semantic Specification Release'"/>
    <!--    owl:priorVersion -->
    <xsl:variable name="priorVersion" select="'3.1.2'"/>
